...
const SuperAdminPortal: React.FC<SuperAdminPortalProps> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#101621] dark:text-slate-100" style={selectedTheme.cssVars}>
      <header className="border-b border-slate-200 bg-white px-6 py-4 shadow-sm dark:border-slate-700/70 dark:bg-[#151c2b]">
      ...
      <main className="mx-auto grid max-w-7xl grid-cols-[300px_1fr] gap-6 p-6">
        <aside className="space-y-3">
          {tenantLoading ? (
            <div className="rounded-lg border border-slate-200 bg-white p-6 text-slate-400 shadow-sm dark:border-slate-700 dark:bg-slate-900/60">
            ...
            <button
              ...
              className={`w-full rounded-lg border bg-white p-4 text-left shadow-sm transition hover:shadow-md dark:bg-slate-900/60 ${selectedTenantId === tenant.id ? 'border-[var(--tenant-primary-border)]' : 'border-slate-200 hover:border-slate-300 dark:border-slate-700 dark:hover:border-slate-600'}`}
            ...
            >
              <div className="flex items-center gap-3">
                {tenant.branding?.logoUrl ? (
                  <img src={tenant.branding.logoUrl} className="h-9 w-9 rounded-md object-contain bg-white p-1" />
                ) : (
        ...
        <section className="space-y-5">
          ...
          <div className="rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-700/70 dark:bg-slate-900/60">
            <div className="flex items-start justify-between border-b border-slate-200 p-5 dark:border-slate-700/70">
              <div>
                <div className="flex items-center gap-3">
                  <h2 className="text-2xl font-black">{selectedTenant?.name || 'Novo tenant'}</h2>
                  {selectedTenant && (
                    <span className={`rounded-full px-2.5 py-1 text-xs font-black ${selectedTenant.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-300'}`}>
              ...
              {selectedTenant && (
                <button
                  ...
                  className="rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-300 dark:hover:bg-slate-800"
                >
            ...
            <div className="flex gap-1 border-b border-slate-200 px-5 pt-4 dark:border-slate-700/70">
              {tabs.map(tab => (
                <button
                  key={tab.id}
            ...
            <div className="p-5">
                  ...
                  {MODULE_CATALOG.map(module => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-lg border p-4 text-left transition ${enabled ? 'border-emerald-300 bg-emerald-50 dark:border-emerald-500/50 dark:bg-emerald-500/10' : 'border-slate-200 bg-slate-50 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-800/50 dark:hover:border-slate-600'} ${module.required ? 'cursor-not-allowed' : ''}`}
                      >
                        ...
                        <p className="mt-4 border-t border-slate-200 pt-3 text-xs font-bold text-slate-500 dark:border-slate-700">{module.impact}</p>
                      </button>
                    );
                  })}
                </div>
              ...
              {activeTab === 'users' && selectedTenant && (
                <div className="space-y-5">
                  ...
                  {showCreateUser && (
                    <form onSubmit={handleCreateUser} className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <input
                          ...
                          className="rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-800 outline-none focus:border-[var(--tenant-primary)] focus:ring-2 focus:ring-[var(--tenant-primary-soft)] dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100"
                        />
                        <input
                          type="email"
                        ...
                        <div className="relative">
                          <input
                            ...
                            minLength={6}
                            className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 pr-10 text-sm font-semibold text-slate-800 outline-none focus:border-[var(--tenant-primary)] focus:ring-2 focus:ring-[var(--tenant-primary-soft)] dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100"
                          />
                          <button
                            type="button"
                        ...
                        <select
                          ...
                          className="rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-800 outline-none focus:border-[var(--tenant-primary)] focus:ring-2 focus:ring-[var(--tenant-primary-soft)] disabled:opacity-50 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100"
                        >
                        ...
                        <div className="flex gap-2">
                          {ACCESSES.map(access => (
                            <button
                              ...
                              className={`h-10 flex-1 rounded-md border text-xs font-black disabled:cursor-not-allowed disabled:opacity-50 ${createUserForm.allowedTypes.includes(access) ? '' : 'border-slate-200 text-slate-500 dark:border-slate-700'}`}
                            >
                              {access}
                            </button>
                          ))}
                        </div>
                        <label className="flex h-10 cursor-pointer items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-xs font-black text-slate-600 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-200">
                          <input
                            type="checkbox"
                  ...
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                    ...
                    <div className="grid grid-cols-[1fr_160px_260px_auto] gap-3">
                      <select value={newUserId} onChange={e => setNewUserId(e.target.value)} className="rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-800 outline-none focus:border-[var(--tenant-primary)] focus:ring-2 focus:ring-[var(--tenant-primary-soft)] dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100">
                      ...
                      <div className="flex gap-2">
                        {ACCESSES.map(access => (
                          <button
                            ...
                            className={`flex-1 rounded-md border text-xs font-black ${newUserAccess.includes(access) ? '' : 'border-slate-200 text-slate-500 dark:border-slate-700'}`}
                          >{access}</button>
                        ))}
                      </div>
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        {tenantMembers.map(member => (
                          <tr key={member.userId}>
                            ...
                            <td className="px-4 py-3">
                              <select value={member.role} onChange={e => updateTenantUser({ tenantId: selectedTenant.id, userId: member.userId, role: e.target.value as AppRole, allowed_types: member.allowed_types, active: member.active })} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-xs font-bold dark:border-slate-700 dark:bg-slate-800">
                                {ROLES.map(role => <option key={role} value={role}>{role}</option>)}
                              </select>
                            </td>
                            <td className="px-4 py-3">
                              <div className="flex gap-2">
                                {ACCESSES.map(access => (
                                  <button type="button" key={access} onClick={() => updateMemberAccess(member.userId, access)} className={`rounded-md border px-2 py-1 text-[10px] font-black ${member.allowed_types.includes(access) ? 'border-emerald-300 bg-emerald-50 text-emerald-700 dark:border-emerald-500/50 dark:bg-emerald-500/10 dark:text-emerald-200' : 'border-slate-200 text-slate-500 dark:border-slate-700'}`}>{access}</button>
                                ))}
                              </div>
                            </td>
                            <td className="px-4 py-3">
                              <button type="button" onClick={() => updateTenantUser({ tenantId: selectedTenant.id, userId: member.userId, role: member.role, allowed_types: member.allowed_types, active: !member.active })} className={`rounded-full px-2.5 py-1 text-xs font-black ${member.active ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-300'}`}>
              ...
              {activeTab === 'branding' && (
                <div className="grid grid-cols-[1fr_260px] gap-5">
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Nome do tenant" className="rounded-md border border-slate-200 bg-white px-4 py-3 text-sm font-semibold outline-none focus:border-[var(--tenant-primary)] focus:ring-2 focus:ring-[var(--tenant-primary-soft)] dark:border-slate-700 dark:bg-slate-800/70" />
                      <input value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })} placeholder="slug" className="rounded-md border border-slate-200 bg-white px-4 py-3 text-sm font-semibold outline-none focus:border-[var(--tenant-primary)] focus:ring-2 focus:ring-[var(--tenant-primary-soft)] dark:border-slate-700 dark:bg-slate-800/70" />
                      <input value={form.displayName} onChange={e => setForm({ ...form, displayName: e.target.value })} placeholder="Nome no app" className="rounded-md border border-slate-200 bg-white px-4 py-3 text-sm font-semibold outline-none focus:border-[var(--tenant-primary)] focus:ring-2 focus:ring-[var(--tenant-primary-soft)] dark:border-slate-700 dark:bg-slate-800/70" />
                      <input value={form.companyName} onChange={e => setForm({ ...form, companyName: e.target.value })} placeholder="Razao/nome comercial" className="rounded-md border border-slate-200 bg-white px-4 py-3 text-sm font-semibold outline-none focus:border-[var(--tenant-primary)] focus:ring-2 focus:ring-[var(--tenant-primary-soft)] dark:border-slate-700 dark:bg-slate-800/70" />
                      <select value={form.defaultBusinessUnit} onChange={e => setForm({ ...form, defaultBusinessUnit: e.target.value as Tenant['defaultBusinessUnit'] })} className="rounded-md border border-slate-200 bg-white px-4 py-3 text-sm font-semibold outline-none focus:border-[var(--tenant-primary)] focus:ring-2 focus:ring-[var(--tenant-primary-soft)] dark:border-slate-700 dark:bg-slate-800/70">
                        <option value="SERVICES">Serviços</option>
                        <option value="PRODUCTS">Produtos</option>
                      </select>
                      <div className="col-span-2 rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/40">
                          ...
                          <div key={group} className="mb-4 last:mb-0">
                            ...
                            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                              {paletteFields.filter(field => field.group === group).map(field => (
                                <label key={field.key} className="grid grid-cols-[42px_1fr] items-center gap-2 rounded-md border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-900/50">
                                  <input
                                    type="color"
                                    value={String(form[field.key])}
                                    onChange={e => setForm({ ...form, [field.key]: e.target.value })}
                                    className="h-8 w-10 rounded border border-slate-200 bg-white p-0 dark:border-slate-700"
                                    title={field.label}
                                  />
                                  <div className="min-w-0">
                  ...
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                    ...
                    <div className="flex h-32 items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white dark:border-slate-700 dark:bg-slate-900/70">
                    ...
                    <label className="mt-4 flex cursor-pointer items-center justify-center gap-2 rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-200 dark:hover:bg-slate-800">
                      <Upload size={16} />
                      Enviar logo
                    ...
                    <div className="mt-5 overflow-hidden rounded-lg border" style={{ borderColor: selectedTheme.borderLight, backgroundColor: selectedTheme.backgroundLight }}>
                      <div className="flex min-h-44">
                        <aside className="w-20 border-r p-3" style={{ borderColor: selectedTheme.borderLight, backgroundColor: selectedTheme.sidebarLight }}>
                          <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-md border bg-white" style={{ borderColor: selectedTheme.primaryBorder }}>
