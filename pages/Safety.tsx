...
const Safety: React.FC<SafetyProps> = ({ data, updateData, globalConfig }) => {
  ...
  const getColors = (index: number) => {
      const colors = [
          { bg: 'bg-[var(--tenant-secondary-soft)]', text: 'text-[var(--tenant-secondary)]', border: 'hover:border-[var(--tenant-secondary-border)]', btnHover: 'hover:bg-[var(--tenant-secondary-soft)]' },
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[var(--tenant-border)] pb-6">
            <div>
                <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                    <HardHat size={28} className="text-[var(--tenant-primary)]" />
            ...
            <div className="flex bg-[var(--tenant-control)] p-1 rounded-lg">
                <button onClick={() => setActiveTab('nrs')} className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'nrs' ? 'bg-[var(--tenant-panel)] shadow text-[var(--tenant-primary)]' : 'text-slate-500 hover:text-slate-700'}`}>Normas (NRs)</button>
                <button onClick={() => setActiveTab('kits')} className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'kits' ? 'bg-[var(--tenant-panel)] shadow text-[var(--tenant-primary)]' : 'text-slate-500 hover:text-slate-700'}`}>Kits de Produto</button>
            </div>
        </header>
                    ...
                    <div className="bg-[var(--tenant-panel)] rounded-lg shadow-sm border border-[var(--tenant-border)] p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                            {nrsList.map(nr => {
                                ...
                                return (
                                    <div key={nr.code} className={`border rounded-lg p-4 transition-all cursor-pointer ${activeItem ? 'border-emerald-500 bg-emerald-50 ring-1 ring-emerald-500' : 'border-[var(--tenant-border)] hover:border-[var(--tenant-border)]'}`} onClick={() => toggleNR(nr.code)}>
                                        <div className="flex justify-between items-start mb-2">
                                            <span className={`font-black text-sm px-2 py-0.5 rounded ${activeItem ? 'bg-emerald-200 text-emerald-800' : 'bg-[var(--tenant-control)] text-slate-600'}`}>{nr.code}</span>
                        ...
                        {(data.safetyCosts || []).filter(s => s.active).length > 0 && (
                            <div className="space-y-3">
                                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-[var(--tenant-border)] pb-2">Custos de Treinamento / Exames (Por Pessoa)</h4>
                                {(data.safetyCosts || []).filter(s => s.active).map(item => (
                                    <div key={item.id} className="flex items-center gap-4 bg-[var(--tenant-control)] p-3 rounded-lg border border-[var(--tenant-border)]">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center gap-1">
                                                ...
                                                <input 
                                                    ...
                                                    className="bg-transparent font-bold text-slate-800 text-sm outline-none w-24 border-b border-[var(--tenant-border)] focus:border-emerald-500"
                                                />
                                            </div>
                                        </div>
                                        <div className="flex-1">
                                            ...
                                            <input 
                                                ...
                                                className="bg-transparent font-bold text-slate-800 text-sm outline-none w-16 border-b border-[var(--tenant-border)] focus:border-emerald-500"
                                            />
                                        </div>
                    ...
                    <div className="bg-[var(--tenant-panel)] rounded-lg shadow-sm border border-[var(--tenant-border)] p-6">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h3 className="font-bold text-slate-800 flex items-center gap-2">
                                    <Package size={20} className="text-[var(--tenant-secondary)]" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            {(globalConfig.kitTemplates || []).map((kit, index) => {
                                ...
                                return (
                                    <div key={kit.id} className={`border border-[var(--tenant-border)] rounded-lg p-5 ${style.border} hover:shadow-md transition-all group`}>
                                        ...
                                        <button onClick={() => addKitToExpenses(kit.id)} className={`w-full py-2 bg-[var(--tenant-control)] text-slate-600 font-bold rounded-lg text-xs ${style.btnHover} hover:text-white transition-colors flex items-center justify-center gap-2`}>
                            ...
                            {(globalConfig.kitTemplates || []).length === 0 && (
                                <div className="col-span-3 text-center py-8 border-2 border-dashed border-[var(--tenant-border)] rounded-lg bg-[var(--tenant-control)]">
                        ...
                        <div className="mt-6 p-4 bg-[var(--tenant-secondary-soft)] rounded-lg flex gap-3 border border-[var(--tenant-secondary-border)]">
                             <Info size={18} className="text-[var(--tenant-secondary)] mt-0.5 shrink-0" />
                             <p className="text-xs text-[var(--tenant-secondary)] leading-relaxed">
