...
import { Plus, Search, FileText, CheckCircle, XCircle, X, Clock, Copy, Edit3, Trash2, PieChart, TrendingUp, Calendar, User, LayoutGrid, List, ArrowRight, DollarSign, Users, Briefcase, GripVertical, ExternalLink, BarChart3, Zap, Repeat, AlertCircle, Snowflake, Filter, Save, Landmark, Package, Activity, History } from 'lucide-react';
...
const CRM: React.FC<CRMProps> = ({
    ...
    const [activeRightPanelTab, setActiveRightPanelTab] = useState<'overview' | 'timeline'>('overview');
    ...
    const filteredProposals = activeProposals.filter(p => {
        ...
        if (businessUnit === 'PRODUCTS') {
            matchesPipeline = p.type === 'PRODUCT';
        } else {
            // In Services BI, we toggle between Continuous and Spot
            matchesPipeline = (pipelineFilter === 'CONTINUOUS' ? p.type === 'CONTINUOUS' : p.type === 'SPOT');
        }
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        ...
                        <button
                            onClick={() => {
                                ...
                                if (clients.length > 0 && !createClientId) {
                                    setCreateClientId(clients[0].id);
                                }
                                setShowCreateModal(true);
                            }}
                            className={`${businessUnit === 'SERVICES' ? 'bg-[#0f172a] dark:bg-indigo-600 hover:bg-[#1e293b] dark:hover:bg-indigo-700 shadow-slate-900/20 dark:shadow-indigo-500/20' : 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-900/20 dark:shadow-emerald-500/20'} text-white px-5 py-3 rounded-lg font-bold shadow-lg flex items-center gap-2 transition-all active:scale-95`}
                        >
                                                ...
                                                >
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded border transition-colors ${prop.type === 'SPOT' ? 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800' : prop.type === 'PRODUCT' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800' : 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800'}`}>
                                                            {prop.type === 'SPOT' ? 'Spot / Consultoria' : prop.type === 'PRODUCT' ? 'Produtos' : 'Contrato Mensal'}
                                                        </span>
                                                    </td>
                                                    <td className="px-6 py-4">
            ...
            {selectedProposal && (
                <div className="w-96 shrink-0 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 shadow-xl flex flex-col h-full animate-in slide-in-from-right duration-300 z-20 transition-colors">
                    <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 shrink-0">
                        <div className="flex items-center justify-between mb-2">
                            <span className={`px-2 py-1 rounded text-xs font-bold ${selectedProposal.type === 'SPOT' ? 'bg-amber-100 text-amber-700' : selectedProposal.type === 'PRODUCT' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>
                                {selectedProposal.type === 'SPOT' ? 'SPOT' : selectedProposal.type === 'PRODUCT' ? 'PRODUTO' : 'MENSAL'}
                            </span>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                            ...
                            <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${selectedProposal.type === 'SPOT' ? 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800' : selectedProposal.type === 'PRODUCT' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800' : 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800'}`}>
                                {selectedProposal.type === 'SPOT' ? 'Spot' : selectedProposal.type === 'PRODUCT' ? 'Produtos' : 'Mensal'}
                            </span>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col">
                        ...
                        {/* TABS */}
                        <div className="flex border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 pt-2 px-6 shrink-0 sticky top-0 z-10 transition-colors">
                            <button
                                onClick={() => setActiveRightPanelTab('overview')}
                                className={`pb-3 px-2 text-[11px] font-bold uppercase tracking-wider transition-colors border-b-2 ${activeRightPanelTab === 'overview' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}`}
                            >
                                Visão Geral
                            </button>
                            <button
                                onClick={() => setActiveRightPanelTab('timeline')}
                                className={`pb-3 px-2 ml-6 text-[11px] font-bold uppercase tracking-wider transition-colors border-b-2 flex items-center gap-1.5 ${activeRightPanelTab === 'timeline' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}`}
                            >
                        ...
                        <div className="p-6 space-y-6 flex-1">
                            ...
                            {activeRightPanelTab === 'timeline' && (
                                <div className="space-y-0 relative before:absolute before:inset-0 before:ml-4 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-300 dark:before:via-slate-700 before:to-transparent">
                                    {(!selectedProposal.timeline || selectedProposal.timeline.length === 0) ? (
                                        ...
                                        [...selectedProposal.timeline]
                                            ...
                                            .map((event, index) => {
                                                let Icon = Activity;
                                                let iconBg = "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-300 dark:border-slate-600";

                                                if (event.type === 'CREATED') {
                                                    Icon = Plus;
                                                    iconBg = "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 border-emerald-300 dark:border-emerald-700";
                                                } else if (event.type === 'STAGE_CHANGE') {
                                                    Icon = TrendingUp;
                                                    iconBg = "bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 border-blue-300 dark:border-blue-700";
                                                } else if (event.type === 'STATUS_CHANGE') {
                                                    Icon = AlertCircle;
                                                    iconBg = "bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400 border-amber-300 dark:border-amber-700";
                                                } else if (event.type === 'VERSION_CREATED') {
                                                    Icon = FileText;
                                                    iconBg = "bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 border-indigo-300 dark:border-indigo-700";
                                                }
                                                ...
                                                return (
                                                    <div key={event.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active mb-6">
                                                        {/* Icon */}
                                                        <div className={`flex items-center justify-center w-8 h-8 rounded-full border border-white dark:border-slate-900 absolute left-0 md:left-1/2 -translate-y-4 sm:translate-y-0 transform -translate-x-1/2 shrink-0 shadow-sm z-10 ${iconBg} transition-colors`}>
                                                            <Icon size={14} />
                                                        </div>

                                                        {/* Card */}
                                                        <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm hover:shadow-md transition-shadow">
                                                            <div className="flex items-center justify-between mb-2">
                                                            ...
                                                            <div className="flex items-center gap-2 mb-2">
                                                                <div className="flex items-center gap-1.5 text-[10px] bg-slate-100 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-full font-medium">
                                                                    <Clock size={10} /> {new Date(event.date).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}
                                                                </div>
                                                            ...
                                                            {event.metadata && (
                                                                <div className="mt-2 text-[11px] text-slate-600 dark:text-slate-300 border-t border-slate-100 dark:border-slate-700 pt-2">
                                                                    {event.type === 'STAGE_CHANGE' && (
                                                                        <div className="flex items-center gap-1">
                                                                            <span className="line-through opacity-60 bg-slate-100 dark:bg-slate-800 px-1 rounded">{event.metadata.from}</span>
                                                                            <ArrowRight size={10} className="text-slate-400" />
                                                                            <span className="font-bold bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 px-1 rounded">{event.metadata.to}</span>
                                                                        </div>
                                                                    )}
                                                                    {event.type === 'STATUS_CHANGE' && (
                                                                        <div className="flex items-center gap-1">
                                                                            <span className="line-through opacity-60 bg-slate-100 dark:bg-slate-800 px-1 rounded">{event.metadata.from}</span>
                                                                            <ArrowRight size={10} className="text-slate-400" />
                                                                            <span className="font-bold bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-1 rounded">{event.metadata.to}</span>
                                                                        </div>
                                                                    )}
                                                                    {event.type === 'VERSION_CREATED' && event.metadata.notes && (
                                                                        <p className="italic bg-slate-50 dark:bg-slate-900/50 p-1.5 rounded text-slate-500 dark:text-slate-400">"{event.metadata.notes}"</p>
                                                                    )}
                                                                </div>
                                                            )}
                                                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border dark:border-slate-800 transition-colors">
                            ...
                            {createStep === 1 && (
                                <div className={`p-6 grid grid-cols-1 ${businessUnit === 'SERVICES' ? 'md:grid-cols-2' : 'md:grid-cols-1'} gap-4`}>
                                    ...
                                    {businessUnit === 'PRODUCTS' && (
                                        <button
                                            onClick={() => { setCreateType('PRODUCT'); setCreateStep(2); }}
