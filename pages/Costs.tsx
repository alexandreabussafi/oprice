...
import { ProposalData, ExpenseItem, CapexItem } from '../types';
import { Plus, Trash2, Truck, Wrench, Shield, Monitor, LayoutGrid, LayoutList, Wand2, Users, Calculator, Info, AlertCircle, Package, Clock, DollarSign, Database, CreditCard, Calendar } from 'lucide-react';
...
const Costs: React.FC<CostsProps> = ({ data, updateData, globalConfig }) => {
    ...
    const addCapex = () => {
        ...
        updateData({ capexItems: [...(data.capexItems || []), newItem] });
    };

    const removeCapex = (id: string) => {
        updateData({ capexItems: (data.capexItems || []).filter((c) => c.id !== id) });
    };

    const updateCapex = (id: string, field: keyof CapexItem, value: any) => {
        const updated = (data.capexItems || []).map((c) => {
            if (c.id === id) return { ...c, [field]: value };
            return c;
        });
        updateData({ capexItems: updated });
    };
    ...
    return (
        <div className="p-8 space-y-6 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800">Custos e Investimentos</h2>
                    <p className="text-slate-500 text-sm mt-1">Gestão de despesas operacionais (OPEX) e ativos imobilizados (CAPEX).</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    {activeTab === 'opex' && globalConfig?.kitTemplates && globalConfig.kitTemplates.length > 0 && (
            ...
            <div className={`flex flex-col xl:flex-row gap-8 items-start ${activeTab === 'opex' ? 'block' : 'hidden'}`}>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-slate-800 text-base">Ativos Imobilizados (CAPEX)</h3>
                                    <p className="text-[11px] text-slate-500 font-medium">Equipamentos, veículos e infraestrutura adquirida para o contrato.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-4">
                            {(data.capexItems || []).length === 0 ? (
                                <div className="p-12 text-center">
                                    ...
                                    <h4 className="text-lg font-bold text-slate-700 mb-2">Nenhum Ativo Registrado</h4>
                                        ...
                                        <div key={capex.id} className="group relative bg-white rounded-xl border border-slate-200 shadow-sm hover:border-indigo-300 transition-all p-3 flex flex-col gap-3">
                                            ...
                                            <div className="pr-6">
                                                <input
                                                    type="text"
                                                    value={capex.name}
                                                    onChange={(e) => updateCapex(capex.id, 'name', e.target.value)}
                                            ...
                                            <div className="grid grid-cols-2 gap-3 mt-1">
                                                <div className="space-y-1 col-span-2">
                                                    ...
                                                    <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 focus-within:border-indigo-400 transition-all">
                                                        ...
                                                        <input
                                                            type="number"
                                                            value={capex.value}
                                                            onChange={(e) => updateCapex(capex.id, 'value', parseFloat(e.target.value) || 0)}
                                                            className="w-full bg-transparent text-sm font-bold text-slate-700 outline-none p-0 border-none focus:ring-0"
                                                        />
                                                    </div>
                                                </div>
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg px-2 py-2">
                                                        <input
                                                            ...
                                                            onChange={(e) => updateCapex(capex.id, 'lifespanMonths', parseInt(e.target.value) || 1)}
                                                            className="w-full bg-transparent text-sm font-bold text-slate-700 outline-none p-0 border-none text-center focus:ring-0"
                                                        />
                                                    </div>
                                                </div>
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg px-2 py-2">
                                                        <input
                                                            ...
                                                            value={capex.purchaseMonth}
                                                            onChange={(e) => updateCapex(capex.id, 'purchaseMonth', parseInt(e.target.value) || 0)}
                                                            className="w-full bg-transparent text-sm font-bold text-slate-700 outline-none p-0 border-none text-center focus:ring-0"
                                                        />
                                                    </div>
                                                </div>
                                                ...
                                                <div className="col-span-2 space-y-1">
                                                    <label className="text-[10px] text-slate-400 font-bold uppercase">Pagamento</label>
                                                    <select
                                                ...
                                                {capex.paymentTerm === 'INSTALLMENTS' && (
                                                    <div className="col-span-2 space-y-1">
                                                        <label className="text-[10px] text-slate-400 font-bold uppercase">Quantidade de Parcelas</label>
                                                        <input
                                                            type="number"
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm space-y-4">
                        <div>
                            <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2 mb-2">
                                ...
                                Impacto no VPL (Valor Presente)
                            </h4>
                            <p className="text-xs text-slate-500 mb-4">
                                O investimento em ativos (CAPEX) reduz drasticamente o caixa nos primeiros meses (período de compra).
                            ...
                            <label className="flex items-start gap-3 p-3 border border-slate-200 rounded-lg hover:bg-slate-50 cursor-pointer transition-colors">
                                ...
                                <div>
                                    ...
                                    <span className="block text-[10px] text-slate-500 leading-tight mt-1">
                                        Se ativo, o valor contábil restante dos equipamentos no último mês do projeto injeta um valor positivo simulado na TIR e VPL.
                                    </span>
                                </div>
                            </label>
                        </div>
