...
const Tooltip: React.FC<{ text: string }> = ({ text }) => (
    <div className="group relative inline-block ml-1 align-middle">
        <Info size={14} className="text-slate-400 hover:text-[var(--tenant-secondary)] cursor-help transition-colors" />
        <div className="invisible group-hover:visible absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-[var(--tenant-panel)] text-white text-xs rounded-lg shadow-xl z-50 text-center leading-relaxed font-normal pointer-events-none">
            {text}
...
const Settings: React.FC<SettingsProps> = ({ data, updateData, resetData }) => {
    ...
    const groupsConfig = [
        ...
        { key: 'groupB', title: 'Grupo B - Tempo não Trabalhado', desc: 'Férias, 13º Salário, Ausências', color: 'blue', bg: 'bg-[var(--tenant-secondary-soft)]', text: 'text-[var(--tenant-secondary)]', border: 'border-[var(--tenant-secondary-border)]' },
        ...
        <div className="p-8 space-y-8 max-w-[1600px] mx-auto">
            <header>
                <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                    <SettingsIcon size={28} className="text-[var(--tenant-primary)]" />
            ...
            <section>
                <h3 className="text-lg font-bold text-slate-700 mb-4 border-b border-[var(--tenant-border)] pb-2">Parâmetros de Custo Indireto</h3>
                    ...
                    <div className="bg-[var(--tenant-panel)] p-6 rounded-lg border border-[var(--tenant-border)] shadow-sm">
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-lg text-white shadow-sm ${currentModel === 'MARKUP' ? 'bg-emerald-600' : 'bg-[var(--tenant-primary)]'}`}>
                                    <TrendingUp size={20} />
                                </div>
                                <div>
                            ...
                            <div className="flex bg-[var(--tenant-control)] p-0.5 rounded-lg">
                                <button
                                    onClick={() => setPricingModel('MARKUP')}
                                    className={`px-3 py-1.5 text-[10px] font-bold rounded-md transition-all ${currentModel === 'MARKUP' ? 'bg-[var(--tenant-panel)] shadow text-slate-800' : 'text-slate-400 hover:text-slate-600'}`}
                                >
                                    Markup
                                </button>
                                <button
                                    onClick={() => setPricingModel('MARGIN')}
                                    className={`px-3 py-1.5 text-[10px] font-bold rounded-md transition-all ${currentModel === 'MARGIN' ? 'bg-[var(--tenant-panel)] shadow text-slate-800' : 'text-slate-400 hover:text-slate-600'}`}
                                >
                                    Margem
                                </button>
                            </div>
                        ...
                        <div className="relative">
                            {currentModel === 'MARKUP' ? (
                                ...
                                <>
                                    <label className="block text-xs font-bold text-[var(--tenant-secondary)] uppercase mb-1">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="bg-[var(--tenant-panel)] p-6 rounded-lg border border-[var(--tenant-border)] shadow-sm">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="p-2 bg-[var(--tenant-secondary-soft)] text-[var(--tenant-secondary)] rounded-lg shadow-sm"><TrendingUp size={20} /></div>
                                <div>
            ...
            <section>
                <h3 className="text-lg font-bold text-slate-700 mb-4 border-b border-[var(--tenant-border)] pb-2">Impostos e Encargos</h3>
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                    ...
                    <div className="xl:col-span-5 space-y-6">
                        ...
                        <div className="bg-[var(--tenant-panel)] rounded-lg shadow-sm border border-[var(--tenant-border)] p-6">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="p-2 bg-[var(--tenant-secondary-soft)] text-[var(--tenant-secondary)] rounded-lg"><FileText size={20} /></div>
                            ...
                            <div className="flex flex-col gap-3">
                                {(['Lucro Real', 'Lucro Presumido'] as const).map(r => (
                                    <button key={r} onClick={() => updateRegime(r)}
                                        className={`w-full py-3 px-4 rounded-lg border transition-all text-sm font-bold text-left flex justify-between items-center ${data.taxConfig.regime === r ? 'border-[var(--tenant-primary)] bg-[var(--tenant-primary)] text-white' : 'border-[var(--tenant-border)] bg-[var(--tenant-panel)] text-slate-600 hover:bg-[var(--tenant-control)]'}`}>
                                        <span>{r}</span>
                        ...
                        <div className="bg-[var(--tenant-panel)] rounded-lg shadow-sm border border-[var(--tenant-border)] p-6">
                            ...
                            <div className="bg-[var(--tenant-control)] border border-[var(--tenant-border)] rounded-lg p-4 text-center">
                        ...
                        <div className="bg-[var(--tenant-panel)] rounded-lg shadow-sm border border-[var(--tenant-border)]">
                            <div className="p-4 border-b border-[var(--tenant-border)] bg-[var(--tenant-control)] flex justify-between items-center">
                            ...
                            <div className="divide-y divide-[var(--tenant-border)]">
                                {data.taxConfig.salesTaxes.map(tax => (
                                    <div key={tax.id} className="p-3 flex items-center justify-between hover:bg-[var(--tenant-control)]">
                                        <div className="flex items-center gap-3">
                                            <button onClick={() => toggleTaxActive('sales', tax.id)} className="text-slate-400 hover:text-slate-600">
                                                {tax.active ? <CheckSquare size={18} className="text-[var(--tenant-primary)]" /> : <Square size={18} />}
                                            </button>
                        ...
                        <div className="bg-[var(--tenant-panel)] rounded-lg shadow-sm border border-[var(--tenant-border)]">
                            <div className="p-4 border-b border-[var(--tenant-border)] bg-[var(--tenant-control)] flex justify-between items-center">
                                ...
                                <span className="text-xs font-bold text-[var(--tenant-secondary)]">{formatPercent(totalIncomeRate)}</span>
                            </div>
                            <div className="divide-y divide-[var(--tenant-border)]">
                    ...
                    <div className="xl:col-span-7">
                        <div className="bg-[var(--tenant-panel)] rounded-lg shadow-sm border border-[var(--tenant-border)] p-6 h-full flex flex-col">
                            ...
                            <div className="space-y-4">
                                {groupsConfig.map((group) => {
                                    ...
                                    return (
                                        <div key={group.key} className={`border rounded-lg transition-all ${isExpanded ? 'border-[var(--tenant-border)] ring-1 ring-[var(--tenant-border)]' : 'border-[var(--tenant-border)] bg-[var(--tenant-panel)]'}`}>
                                            <button onClick={() => toggleGroup(group.key)} className={`w-full flex justify-between items-center p-4 ${isExpanded ? 'bg-[var(--tenant-control)]' : 'bg-[var(--tenant-panel)]'}`}>
                                            ...
                                            {isExpanded && (
                                                <div className="bg-[var(--tenant-panel)] p-2 border-t border-[var(--tenant-border)]">
                                                    {items.map((item) => (
                                                    ...
                                                    <button onClick={() => addComponent(group.key as any)} className="w-full mt-2 py-3 border border-dashed border-[var(--tenant-border)] rounded-lg text-xs font-bold text-slate-400 hover:bg-[var(--tenant-secondary-soft)] flex justify-center items-center gap-2"><Plus size={14} /> Novo Item</button>
                                                </div>
                                            )}
                                        </div>
                                    );
            ...
            {/* Danger Zone / Reset */}
            <section className="pt-8 border-t border-[var(--tenant-border)]">
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-6 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <button
                        ...
                        className="px-6 py-3 bg-[var(--tenant-panel)] border border-red-200 text-red-700 font-bold rounded-lg hover:bg-red-600 hover:text-white hover:border-red-600 transition-all shadow-sm flex items-center gap-2 whitespace-nowrap"
                    >
                        <RefreshCw size={18} />
                        Resetar Lançamentos
                    </button>
