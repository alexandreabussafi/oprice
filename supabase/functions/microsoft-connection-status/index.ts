...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await context.serviceClient
      ...
      .select('id, microsoft_email, microsoft_user_id, scopes, status, sync_enabled, last_synced_at, error_message')
    ...
    return jsonResponse({ error: error instanceof Error ? error.message : 'Erro ao consultar Microsoft.' }, 400);
  }
});
